...
List::List() : right(0), left(0), size(0) {} //конструктор без параметров (исп. список инициализации)
...
List::~List()
{
	...
	else
	{
		while (right->next != 0) //пока не дойдем до нулевого указателя
		{
			...
			right = temp->next; //теперь вершина указывает на следующий за ней элемент,
